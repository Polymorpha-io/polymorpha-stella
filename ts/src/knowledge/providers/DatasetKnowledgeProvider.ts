...
export type DatasetKnowledgeProviderInput = {
  ...
  dataState?: { raw?: import("../../notebook/types").Notebook | unknown & { rows?: unknown[]; columns?: unknown[]; fileName?: string } | null; uploadId?: string | null } | null;
};
...
export class DatasetKnowledgeProvider implements KnowledgeProvider {
  ...
  // If not injected, returns [] — use knowledgeService.indexDatasetProfile() to pre-index.
  ...
  async provide(
    ...
    try {
      ...
      // No local fallback to "@/store/useRagStore" — GitHub-only. If not injected, return [].
      if (!src) return [];
      const ragState = {
        byDataset: src.ragDatasets,
        activeUploadId: src.activeUploadId ?? null,
      } as unknown as { byDataset: Map<string, import("../../lib/rag/types").RagProfileState>; activeUploadId: string | null };
      const dataState = (src.dataState ?? { raw: null, uploadId: null }) as unknown as { raw: import("../../lib/rag/types").RagDatasetProfile & { rows: unknown[]; columns: { name: string; type: string }[]; fileName: string } | null; uploadId: string | null };
